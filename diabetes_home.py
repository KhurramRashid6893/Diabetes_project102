...
def app(diabetes_df):
  ...
  with st.expander('View-Full Data-Set'):
  ...
  with beta_col1:
    if st.checkbox('Display Column Names'):
      st.dataframe(list(diabetes_df.columns))

    # Add a checkbox in the second column. Display the column data-types of 'diabetes_df' on the click of checkbox.
  with beta_col2:
    if st.checkbox("Display the column data-types of diabetes_df"):
      st.dataframe(diabetes_df.astype(str))

    # Add a checkbox in the third column followed by a selectbox which accepts the column name whose data needs to be displayed.
  with beta_col3:
    if st.checkbox("Display the data of selected columns"):
